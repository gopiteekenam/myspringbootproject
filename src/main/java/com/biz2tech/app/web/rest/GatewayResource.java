...
public class GatewayResource {
   ...
   * @GetMapping("/routes")
   * 
   * @Timed public ResponseEntity<List<RouteVM>> activeRoutes() { List<Route> routes =
   * routeLocator.getRoutes(); List<RouteVM> routeVMs = new ArrayList<>(); routes.forEach(route -> {
   * RouteVM routeVM = new RouteVM(); routeVM.setPath(route.getFullPath());
   * routeVM.setServiceId(route.getId());
   * routeVM.setServiceInstances(discoveryClient.getInstances(route.getLocation()));
   * routeVMs.add(routeVM); }); return new ResponseEntity<>(routeVMs, HttpStatus.OK); }
   */
}
